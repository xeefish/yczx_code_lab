...
1. 完成教程 01：运行一次真实模型对话。
2. 完成教程 02：观察真实模型产生的结构化工具请求。
3. 完成教程 03：运行工具结果回传模型的完整 Agent Loop。
4. 在个人练习目录修改案例 01 和案例 03，并记录实际输出。
5. 写下 Agent Loop 的执行顺序，并说明单次模型调用为什么还不是 Agent。

核心开发候选额外完成：

1. 完成教程 04：运行只读代码库 ReAct Agent。
2. 修改任务，让 Agent 阅读 `AGENTS.md` 和当前周任务。
3. 解释路径边界、敏感文件拦截和最大步数分别解决什么风险。

进度较快的成员选做教程 05，理解 Provider、ToolRegistry 和 Agent 的模块边界。
